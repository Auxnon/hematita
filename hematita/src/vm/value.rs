...
pub type NativeFunction<'n> = &'n (dyn Fn(Arc<Table<'n>>, &VirtualMachine<'n>) -> Result<Arc<Table<'n>>, String>
         + Send
         + Sync);
...
impl<'n> Table<'n> {
    ...
    pub fn array_insert(&self, index: i64, mut value: Nillable<'n>) {
        ...
        (index..=(len.max(1) + 1)).for_each(|index| match take(&mut value) {
            NonNil(new) => value = data.insert(Value::Integer(index), new).nillable(),
            Nil => value = data.remove(&Value::Integer(index)).nillable(),
        });
    }
    ...
    pub fn array_remove(&self, index: i64) -> Nillable<'n> {
        ...
        (index..=len)
            ...
            .for_each(|index| match take(&mut value) {
                NonNil(new) => value = data.insert(Value::Integer(index as i64), new).nillable(),
                Nil => value = data.remove(&Value::Integer(index as i64)).nillable(),
            });
        value
    }
    ...
    pub fn index<'qn>(&self, index: &Value<'qn>) -> Nillable<'n> {
        ...
        data.raw_entry()
            .from_hash(hasher.finish(), |check| index == check)
            .map(|(_, value)| value)
            .cloned()
            .nillable()
    }
                ...
                if let Some((highest, _)) = array.last() {
                    (1..=**highest)
                        .map(|index| {
                            array
                                .iter()
                                .find(|value| *value.0 == index)
                                .map(|(_, value)| *value)
                        })
                        .try_for_each(|value| write!(f, "{}{:?}", comma(), value.nillable()))?;
                }

                data.iter().try_for_each(|(key, value)| match key {
                    Value::Integer(_) => Ok(()),
...
mod tests {
    ...
    fn unrelated_get<'m, 'qn, 'n>(
        map: &'m HashMap<Value<'n>, Value<'n>>,
        index: &Value<'qn>,
    ) -> Option<&'m Value<'n>> {
    ...
    fn table_matches(a: &Table, b: &Table) -> bool {
        ...
        data_a.len() == data_b.len()
            && data_a
                .iter()
                .all(|(key, a)| match unrelated_get(&data_b, key) {
    ...
    fn test_lua_value_table() {
        ...
        let string =
            Value::String("this is in a quantum state of being a string, and being an enum".into());
        let raw_value = true;
        ...
        assert_value_eq!(
            ...
            [string] = raw_value, 4},
            Value::Table(
                lua_table! {1, "hello", item = true, [raw_string] = value, 9,
                [string] = raw_value, 4}
                .arc()
            )
        )
