...
where
    I: Iterator<Item = LexerResult<Token>>;
        ...
        match iter.peek().transpose()? {
            ...
            Some(Token::Identifier(_)) => match parse_expression(iter)? {
                ...
                actor => {
                    ...
                    while let Some(Ok(Token::Comma)) = iter.peek() {
                        iter.eat();
                        variables.push(parse_expression(iter)?.try_into()?)
                    }
            ...
            Some(Token::KeywordLocal) => match iter.eat_peek().transpose()? {
                ...
                Some(Token::Identifier(_)) => {
                    ...
                    while let Some(Ok(Token::Comma)) = iter.peek() {
                        iter.eat();
                        variables.push(iter.identifier())
                    }
            ...
            // function actor()
            Some(Token::KeywordFunction) => statements.push(parse_function_statement(iter, false)?),
            ...
            // while
            // repeat
            Some(Token::KeywordWhile | Token::KeywordRepeat) => statements.push(parse_while(iter)?),
            ...
            _ => break Ok(Block(statements)), //Some(_) => break Err(Error(iter.next()))
        }
    }
}
...
pub fn parse_expression_concat<I>(iter: &mut TokenIterator<I>) -> Result<Expression>
where
    I: Iterator<Item = LexerResult<Token>>,
{
...
pub fn parse_expression_unary<I>(iter: &mut TokenIterator<I>) -> Result<Expression>
where
    I: Iterator<Item = LexerResult<Token>>,
{
    Ok(match iter.peek() {
...
pub fn parse_expression_exponent<I>(iter: &mut TokenIterator<I>) -> Result<Expression>
where
    I: Iterator<Item = LexerResult<Token>>,
{
...
pub fn parse_expression_primary<I>(iter: &mut TokenIterator<I>) -> Result<Expression>
where
    I: Iterator<Item = LexerResult<Token>>,
{
    Ok(match iter.peek().transpose()? {
        ...
        // Simple literals
        Some(Token::LiteralNil) => {
            iter.eat();
            Expression::Nil
        }
        Some(Token::LiteralTrue) => {
            iter.eat();
            Expression::True
        }
        Some(Token::LiteralFalse) => {
            iter.eat();
            Expression::False
        }
...
/// parsed.
pub fn parse_expression_inner<I>(
    iter: &mut TokenIterator<I>,
    actor: Expression,
) -> Result<Expression>
where
    I: Iterator<Item = LexerResult<Token>>,
{
    match iter.peek().transpose()? {
        ...
        Some(Token::OpenBracket) => {
            ...
            let result = parse_expression_inner(
                iter,
                Expression::Index {
                    index,
                    indexee: Box::new(actor),
        ...
        match iter.peek().transpose()? {
            ...
            _ => {
                ...
                match iter.next().transpose()? {
                    ...
                    // expr}
                    Some(Token::CloseCurly) => break Ok(Expression::Table { array, key_value }),
...
fn parse_tuple<I>(iter: &mut TokenIterator<I>) -> impl Iterator<Item = Result<String>> + '_
where
    I: Iterator<Item = LexerResult<Token>>,
{
    let mut first = true;
...
pub fn parse_function_statement<I>(iter: &mut TokenIterator<I>, local: bool) -> Result<Statement>
where
    I: Iterator<Item = LexerResult<Token>>,
{
    ...
    Ok(match name_rest {
        None => Statement::LocalFunction {
            name: name_first,
            arguments,
            body,
        },
        Some((name_rest, None)) => Statement::Function {
            name: (name_first, name_rest),
            arguments,
            body,
        },
        Some((name_rest, Some(name))) => Statement::Method {
            class: (name_first, name_rest),
            name,
            arguments,
            body,
        },
...
pub fn parse_function_expression<I>(iter: &mut TokenIterator<I>) -> Result<Expression>
where
    I: Iterator<Item = LexerResult<Token>>,
{
    expect!(iter.next(), Token::KeywordFunction);
    ...
    fn fmt(&self, f: &mut Formatter) -> FMTResult {
        match self {
            ...
            Self::NumericFor {
                ...
                f,
                "for {} = {}, {}, {} do\n{}end",
                variable, first, limit, step, r#do
            ),
            ...
            Self::Assign { variables, values } => {
                once(&variables.0).chain(variables.1.iter()).try_for_each(
                    |variable| match variable {
                        AssignmentTarget::Identifier(identifier) => write!(f, "{}", identifier),
    ...
    fn fmt(&self, f: &mut Formatter) -> FMTResult {
        match self {
            ...
            Self::Table { array, key_value } => {
                ...
                key_value.iter().try_for_each(|KeyValue { key, value }| {
                    if is_first() {
                        write!(f, "[{}] = {}", key, value)
                    } else {
                        write!(f, ", [{}] = {}", key, value)
            ...
            Self::UnaryOperation { operator, operand } => write!(f, "{}{}", operator, operand),
        }
    }
}
...
impl TryFrom<Expression> for AssignmentTarget {
    ...
    fn try_from(value: Expression) -> Result<Self> {
        Ok(match value {
            ...
            Expression::Index { indexee, index } => Self::Index {
                indexee: *indexee,
                index: *index,
            },
